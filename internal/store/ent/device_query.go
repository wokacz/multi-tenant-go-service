...
func (_q *DeviceQuery) QueryChallenges() *TwoFactorChallengeQuery {
...
func (_q *DeviceQuery) WithChallenges(opts ...func(*TwoFactorChallengeQuery)) *DeviceQuery {
	query := (&TwoFactorChallengeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DeviceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Device, error) {
	...
	if query := _q.withChallenges; query != nil {
		if err := _q.loadChallenges(ctx, query, nodes,
			func(n *Device) { n.Edges.Challenges = []*TwoFactorChallenge{} },
			func(n *Device, e *TwoFactorChallenge) { n.Edges.Challenges = append(n.Edges.Challenges, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DeviceQuery) loadChallenges(ctx context.Context, query *TwoFactorChallengeQuery, nodes []*Device, init func(*Device), assign func(*Device, *TwoFactorChallenge)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(twofactorchallenge.FieldDeviceID)
	}
	query.Where(predicate.TwoFactorChallenge(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(device.ChallengesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
