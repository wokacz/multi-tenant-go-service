...
// QueryAvatarOf chains the current query on the "avatar_of" edge.
func (_q *FileQuery) QueryAvatarOf() *UserQuery {
...
func (_q *FileQuery) Clone() *FileQuery {
	...
	return &FileQuery{
		...
		withAvatarOf:     _q.withAvatarOf.Clone(),
...
// WithAvatarOf tells the query-builder to eager-load the nodes that are connected to
// the "avatar_of" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *FileQuery) WithAvatarOf(opts ...func(*UserQuery)) *FileQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAvatarOf = query
	return _q
}
...
func (_q *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	...
	if query := _q.withAvatarOf; query != nil {
		if err := _q.loadAvatarOf(ctx, query, nodes, nil,
			func(n *File, e *User) { n.Edges.AvatarOf = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *FileQuery) loadAvatarOf(ctx context.Context, query *UserQuery, nodes []*File, init func(*File), assign func(*File, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(file.AvatarOfColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
