...
func (_q *MembershipQuery) QueryRoles() *MembershipRoleQuery {
...
func (_q *MembershipQuery) WithRoles(opts ...func(*MembershipRoleQuery)) *MembershipQuery {
	query := (&MembershipRoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MembershipQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Membership, error) {
	...
	if query := _q.withRoles; query != nil {
		if err := _q.loadRoles(ctx, query, nodes,
			func(n *Membership) { n.Edges.Roles = []*MembershipRole{} },
			func(n *Membership, e *MembershipRole) { n.Edges.Roles = append(n.Edges.Roles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MembershipQuery) loadRoles(ctx context.Context, query *MembershipRoleQuery, nodes []*Membership, init func(*Membership), assign func(*Membership, *MembershipRole)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(membershiprole.FieldMembershipID)
	}
	query.Where(predicate.MembershipRole(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(membership.RolesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
