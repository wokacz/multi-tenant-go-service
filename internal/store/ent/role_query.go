...
type RoleQuery struct {
	...
	withMembershipRoles *MembershipRoleQuery
	withInvitationRoles *InvitationRoleQuery
...
// QueryMembershipRoles chains the current query on the "membership_roles" edge.
func (_q *RoleQuery) QueryMembershipRoles() *MembershipRoleQuery {
	query := (&MembershipRoleClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryInvitationRoles chains the current query on the "invitation_roles" edge.
func (_q *RoleQuery) QueryInvitationRoles() *InvitationRoleQuery {
	query := (&InvitationRoleClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withMembershipRoles: _q.withMembershipRoles.Clone(),
		withInvitationRoles: _q.withInvitationRoles.Clone(),
...
// WithMembershipRoles tells the query-builder to eager-load the nodes that are connected to
// the "membership_roles" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RoleQuery) WithMembershipRoles(opts ...func(*MembershipRoleQuery)) *RoleQuery {
	query := (&MembershipRoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withMembershipRoles = query
	return _q
}

// WithInvitationRoles tells the query-builder to eager-load the nodes that are connected to
// the "invitation_roles" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RoleQuery) WithInvitationRoles(opts ...func(*InvitationRoleQuery)) *RoleQuery {
	query := (&InvitationRoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withInvitationRoles = query
	return _q
}
...
func (_q *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	var (
		...
		loadedTypes = [4]bool{
			...
			_q.withMembershipRoles != nil,
			_q.withInvitationRoles != nil,
		}
	)
	...
	if query := _q.withMembershipRoles; query != nil {
		if err := _q.loadMembershipRoles(ctx, query, nodes,
			func(n *Role) { n.Edges.MembershipRoles = []*MembershipRole{} },
			func(n *Role, e *MembershipRole) { n.Edges.MembershipRoles = append(n.Edges.MembershipRoles, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withInvitationRoles; query != nil {
		if err := _q.loadInvitationRoles(ctx, query, nodes,
			func(n *Role) { n.Edges.InvitationRoles = []*InvitationRole{} },
			func(n *Role, e *InvitationRole) { n.Edges.InvitationRoles = append(n.Edges.InvitationRoles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RoleQuery) loadMembershipRoles(ctx context.Context, query *MembershipRoleQuery, nodes []*Role, init func(*Role), assign func(*Role, *MembershipRole)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Role)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(membershiprole.FieldRoleID)
	}
	query.Where(predicate.MembershipRole(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.MembershipRolesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *RoleQuery) loadInvitationRoles(ctx context.Context, query *InvitationRoleQuery, nodes []*Role, init func(*Role), assign func(*Role, *InvitationRole)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Role)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(invitationrole.FieldRoleID)
	}
	query.Where(predicate.InvitationRole(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.InvitationRolesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
