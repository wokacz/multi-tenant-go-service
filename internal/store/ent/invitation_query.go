...
func (_q *InvitationQuery) QueryRoles() *InvitationRoleQuery {
...
func (_q *InvitationQuery) WithRoles(opts ...func(*InvitationRoleQuery)) *InvitationQuery {
	query := (&InvitationRoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *InvitationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Invitation, error) {
	...
	if query := _q.withRoles; query != nil {
		if err := _q.loadRoles(ctx, query, nodes,
			func(n *Invitation) { n.Edges.Roles = []*InvitationRole{} },
			func(n *Invitation, e *InvitationRole) { n.Edges.Roles = append(n.Edges.Roles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *InvitationQuery) loadRoles(ctx context.Context, query *InvitationRoleQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *InvitationRole)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(invitationrole.FieldInvitationID)
	}
	query.Where(predicate.InvitationRole(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(invitation.RolesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
