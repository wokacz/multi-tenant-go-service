...
//
// Membership status lost "invited" when invitations moved to their own table,
...
//
// Every table comes from the Model mixin, so every table has id, created_at and
// updated_at, and the soft-deletable ones have deleted_at. A mixin wired to some
// schemas and not others produces exactly this kind of gap, and nothing at runtime
// complains until something reads a column that is not there.
