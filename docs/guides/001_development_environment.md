...
`task migrate:diff` i testy z `-race` wymagają reszty narzędzi hosta — obraz nie zawiera generatora ent ani lintera, a
detektor wyścigów nie działa na Alpine.
...
`task compose:reset` pyta o potwierdzenie — kasuje wolumeny, więc kolejny start to zimna kompilacja i pusta baza.
...
i nadpisuje wartości z początku tej strony. Uruchomienie binarki z pominięciem Taska wymaga własnego wyeksportowania
zmiennych.
...
patrz [009 Dane rozwojowe](009_seed_data.md). Odmawia uruchomienia z `ENV=production`, bez `-yes` i na bazie, w której
są konta spoza jego własnej domeny.
